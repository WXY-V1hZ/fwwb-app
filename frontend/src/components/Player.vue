        ...
        if (Hls.isSupported()) {
          ...
          const hls = new Hls({
            xhrSetup: function (xhr, url) {
              ...
              if (url.endsWith('.ts')) {
                // 处理样例模式的 TS 文件请求
                if (props.src.includes('/examples/')) {
                  // 从 props.src 中提取样例路径部分来构建正确的 TS 文件 URL
                  const tsFileName = url.substring(url.lastIndexOf('/') + 1);
                  const correctUrl = `${props.src}/${tsFileName}`;
                  xhr.open('GET', correctUrl, true);
                  return;
                }
                // 处理普通模式下的 TS 文件
